      ...
      <div style={{ maxWidth: 1024, margin: "0 auto" }}>
        <div style={{ marginBottom: 64, maxWidth: 640 }}>
          ...
          <h2 style={{
            ...
            From chat to <span style={{ fontStyle: "italic", fontFamily: "Instrument Serif, Georgia, serif" }}>bet</span> in 4 steps
          </h2>
        </div>
        ...
        <div style={{ display: "grid", gap: 1, background: "var(--border)", borderRadius: 16, overflow: "hidden" }}>
          {steps.map((s) => (
            <div key={s.num} style={{
              background: "var(--background)", padding: "32px 32px 32px 32px",
              display: "grid", gridTemplateColumns: "80px 1fr", gap: 24,
              alignItems: "start",
            }}>
              <span className="eyebrow" style={{ color: "var(--seal-400)", paddingTop: 4 }}>
                {s.num}
              </span>
              <div>
                <h3 style={{ fontSize: 18, fontWeight: 600, marginBottom: 8 }}>{s.title}</h3>
                <p style={{ fontSize: 14, lineHeight: 1.7, color: "var(--muted-foreground)" }}>{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
